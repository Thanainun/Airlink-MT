...
class Auth extends Admin_Pages
{
	...
	function login()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$title_header = ($conf_data['admin_header']);
			...
			$this->template	->write_view('content', 'auth/login_form', $data)
							->write('header_logo', ($title_header))
							->write('site-name', 'Administrator Area')
							->write('template_path',$template_url.'/assets/')
							->render();
		}
	}
	...
	function register()
	{
		$config = $this->siteconfigmodel->getConfig('global_config');
		$conf_data = $this->session->_unserialize($config->value);
		$admin_allow = $conf_data['admin_register'] == 'on';
		if ($this->tank_auth->is_logged_in()) {									// logged in
			redirect('');
		...
		} elseif (!$admin_allow) {	// registration is off
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
					...
					if ($email_activation) {									// send "activate" email
						...
						$this->_show_message($this->lang->line('auth_message_registration_completed_2').' '.anchor('admin/login/', 'Login'));
					}
				} else {
					$errors = $this->tank_auth->get_error_message();
					foreach ($errors as $k => $v)	$data['errors'][$k] = $this->lang->line($v);
	...
	function forgot_password()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->template	->write_view('content', 'auth/forgot_password_form', $data)
							->write('header_logo', 'Forgot Password')
							->write('site-name', 'Administrator Area')
							->write('template_path',$template_url.'/assets/')
							->render();
		}
	...
	function reset_email()
	{
		...
		if ($this->tank_auth->activate_new_email($user_id, $new_email_key)) {	// success
			$this->tank_auth->logout();
			$this->_show_message($this->lang->line('auth_message_new_email_activated').' '.anchor('admin/login/', 'Login'));
...
/* Location: ./system/nostradius/controllers/admin/auth.php */