		...
		$data['plan'] = '';
			...
			if($this->topupmodel->UserAndPassExist($data['user_card'],$data['pass_card'])){//ตรวจสอบ user และพาสเวิดร์  บัตรว่ามีอยู่ในระบบหรือไม่
				...
				if($exp == 'exp')
				{
				    ...
				    $data['error'] = "บัตรที่ท่านจะไช้เติมยังไม่อนุญาติให้ไช้งาน ไม่สามารถสามารถนำมาเติมได้ กรุณาติดต่อ ผู้ดูแลระบบ";
				}else{
...
$sql = "UPDATE voucher SET money=money+". $bplan->price ." WHERE username= '".$data['user']."'";
