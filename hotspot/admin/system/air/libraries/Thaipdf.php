...
if(!class_exists('Thaipdf')){
	...
	class CI_Thaipdf
	{
		...
		var $Fontpath;
/*******************************************************************************
*                               Public methods   APPPATH.'fonts/pdf/';                              *
*******************************************************************************/
		function CI_Thaipdf($orientation='P',$unit='mm',$format='A4')
		{
			//Some checks
			$this->_dochecks();
			...
			$this->ws=0;
			$this->charset='tis620';
			$this->Fontpath = set_realpath(APPPATH.'fonts/thaipdf/', FALSE);
			//Standard fonts
	...
	//Define an alias for total number of pages
		...
		function AddFont($family,$style='',$file=''){
			...
			if(isset($this->fonts[$fontkey])) $this->Error('Font already added: '.$family.' ('.$this->Fontpath.') '.$style);
		...
		function SetFont($family,$style='',$size=0) {
			...
			if($this->FontFamily==$family && $this->FontStyle==$style && $this->FontSizePt==$size) return $this;
		...
		function Text($x,$y,$txt) {
			if($this->charset=='utf8') { $txt = $this->utf8_to_tis620($txt); }
			//Output a string
