...
if($post_data['profile']=='time'||$post_data['profile']=='timeout'||$post_data['profile']=='daily'||$post_data['profile']=='monthly'||$post_data['profile']=='packets'||$post_data['profile']=='packets_day'||$post_data['profile']=='packets_month')
	...
	function deleteBillingPlan()
	{
		...
		if(($group->groupname=='20150817000433')||($group->groupname=='20150817000434')||($group->groupname=='20150817000435')) {
				//$this->db->query('delete radusergroup,
				//			voucher,
				//			radcheck,
				//			radreply,							
				//			radacct
				...
				$this->db->query('delete radusergroup. * , voucher . * , radcheck . * , radreply . * , radacct . * 
						FROM billingplan
						LEFT JOIN radusergroup ON billingplan.groupname = radusergroup.groupname
						LEFT JOIN voucher ON billingplan.name = voucher.billingplan
						LEFT JOIN radcheck ON voucher.username = radcheck.username
						...
						LEFT JOIN radacct ON radacct.username = voucher.username
						WHERE billingplan.groupname ='.$this->uri->segment(5));
		} elseif(($group->name!=$register_status['reg_group'])) {
		...
		{  
			if(($group->groupname=='20150817000433')||($group->groupname=='20150817000434')||($group->groupname=='20150817000435')) {
			$output['res'] = true; 
			$output['msg'] = sprintf($this->lang->line('group_message_del_success1'),$group->name);	
			} else {
			$output['res'] = false; 
	...
	$this->gologinmodel->checkOnlinedie($username);
		...
		//สร้างคุณสมบัติ		
					...
					//if(($billingplan->groupname!='bypass')&&($billingplan->groupname!='teacher'))
					//if(($billingplan->groupname!='20150817000433')&&($billingplan->groupname!='20150817000434'))
						//{
						//	$timetofinish_data = array(
						//			'username'=>$user['username'],
		...
		//*************************************************************	
		$voucher_change['personal_id']	= $post_datas['personal_id'];
		$voucher_change['email']	= $post_datas['email'];
		//*************************************************************
		$voucher_change['valid_until']	= $valid['radreply'];
		//*************************************************************
