...
Class Setting extends MY_Admin
{
	...
	function index()
	{
		...
		$this->template	->add_css('elrte/elrte.min.css?'._DATETIME)
				...
				{
					//********************************************************
				$mail_empty = $this->db->get('mail_setting')->result();
				$data['mail_set'] = array(
					...
					'header_mg'=> $this->input->post('head'),
					'sub_mg'   => $this->input->post('sub')
						);

				if(!empty($mail_empty)){
					$this->db->update('mail_setting',$data['mail_set']);
				} else {
					$this->db->insert('mail_setting',$data['mail_set']);
				}
					//********************************************************
